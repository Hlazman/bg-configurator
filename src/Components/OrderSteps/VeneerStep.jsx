  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const veneers = response.data.data.veneers.data;
              ...
              {filteredImgs.map((veneer, index) => (
                <div key={index} style={{ width: 220, margin: '20px 10px' }}>
                  <Card
                    ...
                    style={{
                      border:
                        formData.step2Field === `image${index + 1}.webp`
                          ? '7px solid #f06d20'
                          : 'none',
                    }}
                    onClick={() => handleCardClick('step2Field', `image${index + 1}.webp`)}
                  >
