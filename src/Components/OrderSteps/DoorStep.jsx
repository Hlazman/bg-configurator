...
//+++++++++++++++++++++++++++
  ...
  const handleDoorClick = async (fieldName, doorId) => {
    let productVariantId = formData.step1Field; // Получаем текущий ID ProductVariant
    ...
    if (!productVariantId) {
      ...
      productVariantId = createProductVariantResponse.data.data.createProductVariant.data.id;

    const orderFromURL = window.location.hash.split('/').pop();
    ...
    await axios.post(
      ...
      const updateProductVariantData = {
        ...
        data: {
          // door: doorId
          door: doorId === previousDoorId ? null : doorId, // Изменение на основе предыдущего значения
